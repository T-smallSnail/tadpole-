...
$.extend($.fn, {
    initJerichoTab: function(setting) {
        var opts = $.fn.extend({
        	...
        	//对jerichotab容器（是必需的，一个jQuery选择器字符串格式为'容器'或' #容器）
            renderTo: null,
            //the unique id of jerichotab(is required and unique, not null)
            //对jerichotab独特的ID（是必需的，独特的，不为空）
            uniqueId: null,
                    ...
                    */
                    ...
                    var newTab = $('<li name="'+ps.name+'" class="jericho_tabs tab_selected" style="width:0px"  id="jerichotab_' + curIndex + '" dataType="' + ps.data.dataType + '" dataLink="' + ps.data.dataLink + '">' +
                            '<div class="tab_left"  style="width:' + (opts.tabWidth - 5) + 'px"  >' +
                                (ps.iconImg == '' ? '' : '<div class="tab_icon" style="' + 'background-image:url(' + ps.iconImg + ')' + '">&nbsp;</div>') +
                                '<div class="tab_close">' + (ps.closeable ? '<a href="javascript:" title="关闭">&nbsp;</a>' : '') + '</div>' +
                                '<div class="tab_text" title="' + ps.title + '"  style="width:' + (opts.tabWidth - 52 + (ps.iconImg == '' ? 25 : 0)) + 'px"  >' + ps.title.cut(opts.tabWidth / 10 - 1) + '</div>  ' +
                            '</div>' +
                            '<div class="tab_right">&nbsp;</div>' +
                    ...
                    tabHash.push({
                        ...
                        closeCallback:ps.closeCallback
                    });
    ...
    loadData: function(flag) {
        return this.each(function() {
        	$('.jericho_tab .tab_selected').css('background-color','#f4f6f8');
            ...
            if (holder.length == 0) {
                ...
                //解决谷歌浏览器切换tab页时滚动条消失BUG
                var ifrId = "jerichotabiframe_"+holderId.split("_")[1];
                fixSize(ifrId);
            }
    ...
    applyCloseEvent: function() {
        return this.each(function() {
            ...
            $('.tab_close>a', this).click(function(e) {
            	...
            	//author:wangpeng;
              //截取url参数判断提是否需要提示语信息()
            	var url = $(this).parents(".jericho_tabs").attr("datalink");
            	...
            	if(urlparams){
            		var name = "callback"
               	       ...
               	       if (r != null){
               	    	   	var paramVuale = r[2]; 
	               	    	var index = $(this).parents(".jericho_tabs").index();
	                    	if(my_flag ==false){
	        	            	if(paramVuale && typeof(window.frames[index][paramVuale])=="function"){
	        	            			window.frames[index][paramVuale](tabHash[index]);
	        		            	}else{
	        		            		 removeTtab();
	        		            	}
	                    	}else{
             ...
             /*   //判断回调函数的值是否弹关闭标签页
            	var index = $(this).parents(".jericho_tabs").index();
            	if(my_flag ==false){
	            	if(tabHash[index].closeCallback){
	            	var callback=tabHash[index].closeCallback;
	            		//tabHash[index].closeCallback(tabHash[index]);
	            	
	            		window.frames[index][callback](tabHash[index]);
		            	}else{
		            		 removeTtab();
		            	}
            	}else{
            ...
            $(this).RightMenu('jerichotabmenu',{menuList:[
                ...
                {menuName:"关闭当前",clickEvent:"$('.tab_selected .tab_close>a').click();"
                ...
                {menuName:"关闭其它",clickEvent:"$('.tab_unselect .tab_close>a').click();"
                	+"setTimeout('$.fn.jerichoTab.resize();setTimeout(\\\'$.fn.jerichoTab.resize();"
                	+"setTimeout(\\\\\\'$.fn.jerichoTab.resize();\\\\\\',1000);\\\',500);',500);"},
                {menuName:"关闭所有",clickEvent:"$('.tab_close>a').click();"
                	+"setTimeout('$.fn.jerichoTab.resize();setTimeout(\\\'$.fn.jerichoTab.resize();"
                	+"setTimeout(\\\\\\'$.fn.jerichoTab.resize();\\\\\\',1000);\\\',500);',500);"}
        	]});
        });
...
function fixSize(iframeId) {
    ...
    iframe.style.height = parseInt(iframeHeight)-1+"px";	
    iframe.scrollWidth;
