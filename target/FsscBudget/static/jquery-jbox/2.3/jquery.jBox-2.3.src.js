...
(function ($) {
    $.jBox = function (content, options) {
        ...
        if (options.showFade) {
            if ((isIE6 && $('iframe').length > 0) || $('object, applet').length > 0) {
                ...
                boxHtml.push('<div id="jbox-fade" class="jbox-fade" style="position:absolute;background-color:#777"></div>');
            }
        }
        ...
        boxHtml.push('<div class="jbox-help-button jbox-button-panel" style="height:50px;padding:5px 0 5px 0;display:none;"></div>');
        ...
        if (withTitle) {
            ...
            boxHtml.push('<div class="jbox-title' + (options.showIcon == true ? '' : (options.showIcon == false ? '' : ' ' + options.showIcon)) + '" style="float:left; width:' + titleWidth + '; line-height:' + (/msie/.test(navigator.userAgent.toLowerCase()) ? 25 : 24) + 'px; padding-left:' + (options.showIcon ? 0 : 5) + 'px;overflow:hidden;text-overflow:ellipsis;word-break:break-all;">' + (options.title == '' ? '&nbsp;' : options.title) + '</div>');
            boxHtml.push('</div>');
        }
        ...
        $.each(content, function (stateName, stateOptions) {
            ...
            states.push('<div class="jbox-button-panel" style="line-height: 40px;height:40px;padding:5px 0 5px 0;text-align: right;' + (withButtons ? '' : 'display:none;') + '">');
            if (!options.isTip) {
        ...
        var setFade = function () {
            if (options.showFade) {
            	if(!($(/msie/.test(navigator.userAgent.toLowerCase()) ? 'html' : 'body').attr('style'))){
	                $.jBox.FadeBoxCount++;
	                $(/msie/.test(navigator.userAgent.toLowerCase()) ? 'html' : 'body').attr('style', 'overflow:hidden;');
            	}
                $boxFade.css({
                    position: "absolute",
        ...
        var removeBox = function () {
        	if (options.showFade) {
                if ($.jBox.FadeBoxCount == 1 || ($.jBox.FadeBoxCount < 1 && $(/msie/.test(navigator.userAgent.toLowerCase()) ? 'html' : 'body').attr('style'))) {
        ...
        var removeBoxImpl = function () {
            ...
            if (data.type != 'IFRAME') {
                $box.find('#jbox-iframe').attr({ 'src': 'about:blank' });
            	 ...
            	 if($box)$box.find('#jbox-iframe').remove();
            }
            $box.html('').remove();
    ...
    $.jBox.tipDefaults = { content: '', icon: 'info', top: '40%', width: 'auto', height: 'auto', opacity: 0, timeout: 2000, closed: function () { } };
    $.jBox.messagerDefaults = { content: '', title: 'jBox', icon: 'none', width: 350, height: 'auto', timeout: 2000, showType: 'slide', showSpeed: 600, border: 0, buttons: {}, buttonsFocus: 0, loaded: function () { }, submit: function (v, h, f) { return true; }, closed: function () { } };
    ...
    $.jBox.close = function (token, boxType) {
        ...
        if (typeof token == 'string') {
            ...
            if(boxs){
            	var iframe=boxs.find('#jbox-iframe');
    ...
    $.jBox.prompt = function (content, title, icon, options) {
        ...
        var html = [];
        html.push('html:');
        html.push('<div style="margin:10px;' + minHeight + 'padding-left:' + (options.icon == 'none' ? 0 : 10) + 'px;text-align:left;padding-top:10px;padding-bottom:10px;">');
        html.push('<span class="jbox-icon jbox-icon-' + options.icon + '"  style="vertical-align: middle;display: inline-block;width:32px; height:32px;"></span>');
    ...
    $.jBox.tip = function (content, icon, options) {
        ...
        var html = [];
        html.push('html:');
        html.push('<div style="min-height:18px;height:auto;margin:10px;padding-left:30px;padding-top:0px;text-align:left;padding-right:10px;">');
        html.push('<span class="jbox-icon jbox-icon-' + options.icon + '" style="vertical-align: middle;display: inline-block;width:32px; height:32px;"></span>');
        html.push(options.content);
        html.push('</div>');
        options.content = html.join('');
