...
<div class="input-append ">
	...
	<input id="${id}Name" name="${labelName}" ${allowInput?'':'readonly="readonly"'} type="text" callBack="${callBack}"  value="${labelValue}" data-msg-required="${dataMsgRequired}"
		class="${cssClass} defined-search" style="${cssStyle} border-radius:4px;    background-size: 16px 16px; background-position: 96% 6px;"/>
...
<script type="text/javascript">
	if("${dataMsgRequired}"=="")$("#${id}Name").data("msg-required","必填信息");
	$("#${id}Button, #${id}Name").click(function(){
		...
		// 正常打开
		top.$.jBox.open("iframe:${ctx}/tag/treeselect?url="+encodeURIComponent("${url}")+"&module=${module}&checked=${checked}&extId=${extId}&isAll=${isAll}&queryParms="+parms, "选择${title}", 300, 420, {opacity: 0.5,
			ajaxData:{selectIds: $("#${id}Id").val()},buttons:{"确定":"ok"}, submit:function(v, h, f){
				if (v=="ok"){
