...
<head>
	...
	/* 	alert("${ctx}${url}"); */
		var tree, setting = {view:{selectedMulti:false,dblClickExpand:false},check:{enable:"${checked}",nocheckInherit:true},
				async:{enable:(type==4),url:"${ctx}${url}${fn:indexOf(url,'?')==-1?'?':'&'}&extId=${extId}&isAll=${isAll}&module=${module}&queryParms=${queryParms}",autoParam:["id", "name=n", "level=lv"]},
	...
	//		key.bind("focus", focusKey).bind("blur", blurKey).bind("change cut input propertychange", searchNode);
		...
		var search_setting = {view:{selectedMulti:false,dblClickExpand:false},check:{enable:"${checked}",nocheckInherit:true},
		...
		function searchNode() {
			...
			var rootUrl = "${url}";
			var search_queryParms = "${queryParms}";
			var search_id = "${id}";
			if (rootUrl.indexOf("/asynctreeDataCompany") > -1 && search_queryParms.length == 0 && search_id.length == 0
				|| rootUrl.indexOf("/asynctreeDataOnlyDept") > -1) { //筛选asynctreeDataCompany的情况
			
				var results;
				var value = encodeURI(encodeURI($.trim(key.get(0).value)));  //两次是为了解决乱码问题
				...
				if (value.length>0) { //如果key不为空，则查询
					$.ajax({
				    	url: "${ctx}"+"/sys/org/asynctreeDataCompanySearch?"+"key=" + value + "&queryParms=" + search_queryParms,
				        dataType: "json",
				        type: 'get',
				        success: function(data) {
				        	...
				        	tree = $.fn.zTree.init($("#tree"), search_setting, results);
							tree.expandAll(true);
				      	}
				    });
				} else {
