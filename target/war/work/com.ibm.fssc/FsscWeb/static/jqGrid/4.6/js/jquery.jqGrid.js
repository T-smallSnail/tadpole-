...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		$("<div class='loading ui-state-default ui-state-active' id='load_"+this.id+"'></div>").insertBefore(gv);
		...
		setPager = function (pgid, tp){
			...
			// 增加 <td id='"+rgt+"' align='right'></td>，以显示总记录数 modify by Lovin
			var rgtr = "";
			if(ts.p.viewrecords) {
				rgtr = "<td id='"+rgt+"' align='left'></td>"
			}
			$("#"+$.jgrid.jqID(pgid) )
			...
			//修改，去掉查找，刷新功能
			// 增加 <td id='"+rgt+"' align='right'></td>，以显示总记录数 modify by Lovin
			.append("<div id='"+pgcnt+"' class='ui-pager-control' role='group'><table cellspacing='0' cellpadding='0' border='0' class='ui-pg-table' style='width:100%;table-layout:fixed;height:100%;' role='row'><tbody><tr>"+ rgtr+"<td id='"+cent+"' align='left' style='white-space:pre;'></td>"  + "</tr></tbody></table></div>")
			...
			if(dir==="rtl") { pgl += str; }
			if(ts.p.pginput===true) { pginp= "<td dir='"+dir+"'>"+$.jgrid.format(ts.p.pgtext || "","当前 <input class='ui-pg-input' type='text' size='2' maxlength='7' value='0' role='textbox'/> 页 /","<span id='sp_1_"+$.jgrid.jqID(pgid)+"'></span>")+"</td>";}
			if(ts.p.pgbuttons===true) {
				var po=["first"+tp,"prev"+tp, "next"+tp,"last"+tp]; if(dir==="rtl") { po.reverse(); }
				pgl += "<td id='"+po[0]+"' class='ui-pg-button ui-corner-all'><span style='margin-right:10px;'><i class='ui-icon ui-icon-seek-first'></i></span></td>";
				pgl += "<td id='"+po[1]+"' class='ui-pg-button ui-corner-all'><span><i class='ui-icon ui-icon-seek-prev'></i></span></td>";
				pgl += pginp !== "" ? sep+pginp+sep:"";
				pgl += "<td id='"+po[2]+"' class='ui-pg-button ui-corner-all'><span style='margin-right:10px;'><i class='ui-icon ui-icon-seek-next'></i></span></td>";
				pgl += "<td id='"+po[3]+"' class='ui-pg-button ui-corner-all'><span><i  class='ui-icon ui-icon-seek-end'></i></span></td>";
		...
		$(this).addClass('ui-jqgrid-btable table table-bordered table-striped table-hover').append(firstr);
		firstr = null;
...
$.jgrid.extend({
	...
	setGridWidth : function(nwidth, shrink) {
		return this.each(function(){
			...
			$($t.grid.bDiv).css("width",nwidth+15+"px");
			$($t.grid.hDiv).css("width",nwidth+15+"px");
...
$.jgrid.extend({
	...
	navGrid : function (elem, o, pEdit,pAdd,pDel,pSearch, pView) {
		...
		return this.each(function() {
			...
			for(i = 0; i<clone; i++) {
				...
				sep = "<td class='ui-pg-button ui-state-disabled' style='width:0px;'><span class='ui-separator'></span></td>",
				pgid, elemids;
				if(i===0) {
					pgid = elem;
					elemids = $t.p.id;
	...
	navSeparatorAdd:function (elem,p) {
		...
		return this.each(function() {
			...
			if(findnav) {
				var sep = "<td class='ui-pg-button ui-state-disabled' style='width:0px;'><span class='"+p.sepclass+"'></span>"+p.sepcontent+"</td>";
				if (p.position === 'first') {
					if (findnav.rows[0].cells.length === 0) {
